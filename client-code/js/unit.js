...
function startUnit(color, gx, gy, num){
  ...
  this.rect = c.rect(25+50*gx, 75+50*gy,
      50,50).attr({"fill":qpo.COLOR_DICT[color],"opacity":0.7});
...
function finishUnit(unit){
  ...
  unit.kill = function(){
    ...
    if(activeScreen == "game"){ //update game GUI
      switch(unit.team){
        case "red":
          redDead++;
          break;
  ...
  unit.moveLeft = function(){
    ...
    if (unit.rect.attr('x') > qpo.guiCoords.gameBoard.leftWall || activeScreen == "menu") {
      unit.rect.stop();
  ...
  unit.moveRight = function(){
    if (unit.rect.attr('x') + unit.rect.attr('width') < qpo.guiCoords.gameBoard.rightWall || activeScreen == "menu") {
      /*
      unit.rect.stop();
      var anim = Raphael.animation( {"x":unit.rect.attr('x') + qpo.guiDimens.columns*qpo.guiDimens.squareSize},
    ...
    switch(unit.team){
      case "blue":
        shot = c.rect(unit.rect.attr('x') + 22,
                      ...
                      6, 2);
        anim = Raphael.animation({"height":25, "y": shot.attr('y') + 0}, 500*qpo.timeScale, function(){
          shot.animate({"y": shot.attr('y') + 125*7}, 3000*7*qpo.timeScale);
        });
        if (unit.movingForward){
          anim = Raphael.animation({"height":25, "y": shot.attr('y') + qpo.guiDimens.squareSize/6 + 10}, 500*qpo.timeScale, function(){
            shot.animate({"y": shot.attr('y') + 125*7}, 3000*7*qpo.timeScale);
          });
        }
        break;
      case "red":
        shot = c.rect(unit.rect.attr('x') + 22,
                      unit.rect.attr('y') - 4,
                      6, 2);
        anim = Raphael.animation({"height":25, "y": shot.attr('y') - 25}, 500*qpo.timeScale, function(){
          shot.animate({"y": shot.attr('y') - 125*7}, 3000*7*qpo.timeScale);
        });
        if (unit.movingForward){
          anim = Raphael.animation({"height":25, "y": shot.attr('y') - 25 - qpo.guiDimens.squareSize/6 - 10}, 500*qpo.timeScale, function(){
            shot.animate({"y": shot.attr('y') - 125*7}, 3000*7*qpo.timeScale);
          });
        }
        break;
