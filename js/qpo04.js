...
function startBomb(su){ //su = source unit
  ...
  this.number = findSpot();
  console.log("bomb " + this.number + " created, " + bombs[this.number]);
  return this;
}
function improveBomb(bo){
  //make it purple:
  bo.phys.attr({"fill":COLOR_DICT["bomb color"],
             "opacity":.5,
  ...
  bo.explode = function(){
    bo.exploded = true;
    var cx = bombs[num].phys.getBBox().x;
    var cy = bombs[num].phys.getBBox().y;
    bombs[num].phys.stop();
    ...
    },3000*timeScale,function(){bombs[num].destroy()});
    bombs[num].phys.animate(anim);
  }
  //give the bomb class a "destroy()" method:
  bo.destroy = function(){
    bombs[num].phys.remove();
    bombs[num]=undefined;
    console.log("bomb " + num + " destroyed, " + bombs[num]);
  }
}
function finishBomb(bo){
  var by = bo.phys.attr('y');
  var num = bo.number;
  //animate it based on what team it's on:
  switch(bo.team){
    case "blue":
      bo.phys.animate({'y':by+150},9000,function(){
        console.log("bo.number: " + bo.number + "... bomb " + num + " exploded");
        bombs[num].explode()});
      break;
    case "red":
      bo.phys.animate({'y':by-150},9000,function(){
