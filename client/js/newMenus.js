  ...
  this.close = function(status, time){ //clear the canvas and open the next screen
    ...
    qpo.fadeOut(this.all, function(){
      ...
      switch(status){
        ...
        //Spectator mode!
        case 'Spectate': {
          //getSpectatables creates the Spectate menu, so run that first
          //It has to be this way, because menu options are generated by GET request to server
         qpo.getSpectatables()
          .then(function(data) {
            console.log("Got data ", data, " from getSpectatables");

            qpo.makeSpectateOptions(data)
